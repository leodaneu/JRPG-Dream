...
for (var _i = index_; _i <= index_ + 4; _i++) {
    if (_i >= 0 and _i < ds_list_size(options_)) {
		...
		var _is_equiped = string_pos(_obj_stats.equipment_.weapon_, options_[| index_][? "text"]);
		...
		if (_i == index_ and _is_equiped == 0) 
		 ...
		 //draw_text_colour(x + 4, y + 6 + 10 * (_i), string_hash_to_newline(_option[? "text"]), _color, _color, _color, _color, 1);
